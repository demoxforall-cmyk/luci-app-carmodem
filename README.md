...
### Watchdog (`admin/modem/watchdog`)
Авто-восстановление связи: следит за интернетом (ping) и при залипании (сигнал есть, интернета нет N циклов) чинит эскалацией **L1** реконнект интерфейса → **L2** `mmcli --reset` → **L3** перезагрузка роутера (opt-in, 1 раз за эпизод). Правило «нет сигнала → бездействие». Опциональный хук перезапуска сервиса (напр. `podkop`) при возврате интернета. Наглядная лестница правил + живой маркер strike + журнал событий; состояние из ModemManager, всё изменчивое — на tmpfs. `procd`-сервис, конфиг в UCI `/etc/config/carmodem`.
...
scp -O luci-app-carmodem-3.9-r1.apk root@192.168.1.1:/tmp/
# 2) поставить без сертификата (зависимости подтянутся сами; можно из веб-терминала LuCI):
apk add --allow-untrusted /tmp/luci-app-carmodem-3.9-r1.apk
