...
const files = ["carmodem.js", "view/carmodem/info.js", "view/carmodem/control.js", "view/carmodem/sms.js", "view/carmodem/watchdog.js"];
...
const MAP = {
  ...
  "Traffic ↓ / ↑": "Трафик ↓ / ↑",
  "Type": "Тип",
  "Modem watchdog": "Сторож модема",
  "Auto-recovery": "Автовосстановление",
  "Watchdog": "Сторож",
  "Run check now": "Проверить сейчас",
  "Reset counter": "Сбросить счётчик",
  "Safety rule: with no signal / not registered (covered parking) the watchdog does nothing — it never reconnects or reboots blindly.": "Правило безопасности: при отсутствии сигнала / без регистрации (крытая парковка) сторож бездействует — не реконнектит и не перезагружает вслепую.",
  "On": "Вкл",
  "Off": "Выкл",
  "Escalation rules": "Правила эскалации",
  "While registered but with no internet, the strike counter grows every cycle. One action per cycle, checked top-down; the counter resets only when internet returns.": "Пока модем зарегистрирован, но интернета нет, счётчик strike растёт каждый цикл. Одно действие за цикл, проверка сверху вниз; счётчик сбрасывается только при возврате интернета.",
  "L1 · Reconnect interface": "L1 · Реконнект интерфейса",
  "ifdown/ifup of the WAN interface — the softest fix.": "ifdown/ifup WAN-интерфейса — самое мягкое действие.",
  "L2 · Reset modem": "L2 · Reset модема",
  "mmcli --reset (coordinated with ModemManager, not a raw AT command).": "mmcli --reset (координированно с ModemManager, не сырой AT).",
  "L3 · Reboot router": "L3 · Перезагрузка роутера",
  "Full router reboot — only when the modem is connected (bearer up) but has no data, once per episode.": "Полный ребут роутера — только когда модем подключён (bearer поднят), но данных нет; 1 раз за эпизод.",
  "reboot": "ребут",
  "cooldown": "пауза",
  ...
  "up": "есть",
  "down": "нет",
  "rebooted this episode": "был ребут в эпизоде",
  "clean": "чисто",
  "Settings": "Настройки",
  "Check interval (s)": "Интервал проверки (с)",
  "L1 threshold (cycles)": "Порог L1 (циклы)",
  "L2 threshold (cycles)": "Порог L2 (циклы)",
  "L3 threshold (cycles)": "Порог L3 (циклы)",
  "Reset cooldown (s)": "Пауза между reset (с)",
  "Ping hosts": "Хосты для ping",
  "Allow router reboot (L3)": "Разрешить ребут роутера (L3)",
  "L3 reboots the whole router. Kept safe: only when connected (bearer up) but no data, and once per episode.": "L3 перезагружает весь роутер. Защита: только когда модем подключён (bearer поднят), но данных нет, и 1 раз за эпизод.",
  "WAN interface (for L1 dial)": "WAN-интерфейс (для L1 dial)",
  "Restart service after recovery": "Перезапустить сервис после восстановления",
  "Optional: init.d service restarted once when internet returns — e.g. “podkop”, which otherwise starts blind with no network.": "Опционально: init.d-сервис перезапускается 1 раз при возврате интернета — напр. «podkop», который иначе стартует вслепую без сети.",
  "WAN netdev override": "WAN-netdev (override)",
  "blank = auto": "пусто = авто",
  ...
  "Disabled": "Выключен",
  "Watching": "Наблюдает",
  "No signal — idle": "Нет сигнала — бездействие",
  "Escalating": "Эскалация",
  "reconnect": "реконнект",
  "modem reset": "reset модема",
  "router reboot": "ребут роутера",
  "cycles left": "циклов осталось",
  "all levels reached": "все уровни пройдены",
  "Yesterday": "Вчера",
  "current": "текущая",
